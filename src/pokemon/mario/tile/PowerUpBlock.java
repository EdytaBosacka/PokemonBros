...
public class PowerUpBlock extends Tile {
	...
	public PowerUpBlock(int x, int y, int width, int height, boolean solid, ID id, Handler handler, Sprite picture, int type) {
	...
	public void draw(Graphics graph) {
		if(!poppedUp) {  
			graph.drawImage(picture.getBufferedImage(),x,spriteY,width,height,null);}
	...
	public void update() {
		if(activated&&!poppedUp) {
			...
			if(spriteY<=y-height)
			{
				...
				{
				handler.addEntity(new Bluestar(x,spriteY,width,height,ID.mushroom,handler));
				}
				poppedUp = true;
			}
		}
